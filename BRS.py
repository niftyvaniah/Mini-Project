...
class Dashboard(tk.Frame):
    ...
    def __init__(self, parent, controller):
        ...
        def connectAuthor():
            ...
            cur = conn.cursor()
            cur.execute("PRAGMA foreign_keys = ON")
            cur.execute("CREATE TABLE IF NOT EXISTS authors (BookNumber VARCHAR(10) PRIMARY KEY, Author VARCHAR(100) NOT NULL)")
            conn.commit() 
            conn.close()
        ...
        def connectGenre():
            ...
            cur = conn.cursor()
            cur.execute("PRAGMA foreign_keys = ON")
            cur.execute("CREATE TABLE IF NOT EXISTS genres (BookNumber VARCHAR(10) PRIMARY KEY, Genre VARCHAR(100) NOT NULL)")
            conn.commit() 
            conn.close()
...
class Books(tk.Frame):
     ...
     def __init__(self, parent, controller):
        ...
        self.btnAddBook = Button(self, image=self.add,  bd=0 ,command=addBook)
        ...
        self.btnUpdateStatus.place(x=550,y=640)
...
class Genres(tk.Frame):
     ...
     def __init__(self, parent, controller):
        ...
        self.btnShowall = Button(self, image = self.show, bd=0,command=showAll)
        self.btnShowall.place(x=200,y=105)
        self.btnShowall.config(cursor= "hand2")

        self.btnDeleteGenre = Button(self, image=self.delete, bd=0, command = deleteGenre)
        self.btnDeleteGenre.place(x=340,y=105)
     ...
     def __init__(self, parent, controller):
        ...
        def addReport():
            if BNumber.get() == "" or RDate.get() == "":
                ...
                cur4 = conn.cursor()
                cur.execute("SELECT * FROM borrower")
                cur2.execute("SELECT books.*, GROUP_CONCAT(authors.Author,'|'), GROUP_CONCAT(genres.Genre, '|') FROM books\
                            INNER JOIN authors ON books.BookNumber = authors.BookNumber \
                            INNER JOIN genres ON authors.BookNumber = genres.BookNumber\
                            GROUP BY authors.Author, genres.Genre;")
